...
and gain), narma.png. --replot redraws them from runs.csv and linear_baseline.csv.
"""
import argparse
import warnings
...
def main():
    ...
    if args.replot:
        runs, baseline = pd.read_csv(out / "runs.csv"), pd.read_csv(out / "linear_baseline.csv")
    ...
    plt.close(plotting.plot_narma(grid, baseline["nrmse"].mean(), noise=cfg.memory.readout_noise,
                                  path=out / "narma.png", title=f"NARMA-10 error at each gain, {run_label(cfg)}"))
